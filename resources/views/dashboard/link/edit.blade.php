...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form method="post" action="{{ url('dashboard/links/' . $link->id) }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="main_filter_selector" value="{{ $link->main_filter_selector }}"
                                class="form-control" />
                        </div>
                    </div>
                </div>
                            ...
                            <select name="website_id" class="form-control">
                                ...
                                @foreach ($websites as $website)
                                    <option value="{{ $website->id }}"
                                        {{ $website->id == $link->website_id ? 'selected' : '' }}>
